...
} from '../../../core/offline/backup/offline-backup.types';
    ...
    <section class="backup-page">
      @if (!enabled()) {
        <mat-card appearance="outlined">
          <p>
            La bandera de funcionalidad offlineBackupV1Enabled está desactivada para esta ejecución.
          </p>
        </mat-card>
      } @else {
        <mat-card appearance="outlined">
        ...
        <mat-card appearance="outlined">
          ...
          <p>
            La restauración reemplaza los datos locales, rehidrata la ejecución y fuerza
            reautenticación antes de la próxima sincronización.
          </p>
...
export class BackupPageComponent {
  ...
  async onFileSelected(event: Event) {
    ...
    try {
      ...
      this.feedbackMessage.set(
        `Restauración completada. ${result.imagesRestored} imagen(es) restauradas y reautenticación requerida antes de la próxima sincronización.`,
      );
    } catch (error) {
  ...
  private async exportBackup(includeImages: boolean) {
    ...
    try {
      ...
      this.feedbackMessage.set(
        includeImages
          ? 'Respaldo completo exportado con digest SHA-256 verificable.'
          : 'Respaldo exportado sin binarios ANIMAL_IMAGE.',
      );
    } catch (error) {
      this.errorMessage.set(resolveBackupErrorMessage(error));
    } finally {
...
function resolveBackupErrorMessage(error: unknown) {
  ...
  return 'No pudimos completar la operación de respaldo local.';
}
