...
**PR boundary**: PR3 / Phase 4 + Phase 6 slice — header online/offline indicator, GANADERO sidebar cleanup, and GANADERO manual sync/backups/conflicts route redirects. No animal page/table/media badges in this run.
...
## Test Summary
- **Total tests written this slice**: 5 new/updated behavior groups (2 header, 2 sidebar, 1 route policy)
- **Total tests passing**: 102 in required focused run
- **Layers used**: Unit, component unit, route unit
- **Approval tests**: Existing 19-test layout/route safety net preserved before behavior changes
- **Pure functions created**: 0

## Tests Run
1. Safety net: `PATH="$HOME/.nvm/versions/node/v20.19.6/bin:$PATH" npm test -- --include src/app/ui/layout/main-layout/header/header.spec.ts --include src/app/ui/layout/main-layout/sidebar/sidebar.spec.ts --include src/app/ui/layout/main-layout/sidebar/sidebar.integration.spec.ts --include src/app/app.routes.spec.ts --watch=false` → 4 files / 19 tests passed
2. RED: same layout/route command → 4 files / 15 passed, 6 failed for missing header indicator, GANADERO menu cleanup, and route redirect policy
3. GREEN targeted: same layout/route command → 4 files / 21 tests passed
4. Required focused: `PATH="$HOME/.nvm/versions/node/v20.19.6/bin:$PATH" npm test -- --include src/app/core/offline --include src/app/ui/layout --include src/app/app.routes.spec.ts --watch=false` → 20 files / 102 tests passed

## Deviations
None — implementation follows the design choice to hide manual tooling only from GANADERO while keeping admin/support conflict tooling intact. Direct GANADERO URLs redirect to dashboard via `canMatch`, one of the allowed spec outcomes.

## Remaining Tasks
- [ ] Phase 5: Animal list/detail/media badges
- [ ] 7.2 Run final targeted `ng test` for the complete modified feature/UI scope before verify

## Files Changed
- `hato-fe/src/app/ui/layout/main-layout/header/header.ts` — consumes `OfflineStatusService` and exposes connectivity signals
- `hato-fe/src/app/ui/layout/main-layout/header/header.html` — renders persistent online/offline header indicator with Spanish labels
- `hato-fe/src/app/ui/layout/main-layout/header/header.scss` — styles online/offline connectivity chip responsively
- `hato-fe/src/app/ui/layout/main-layout/header/header.spec.ts` — covers online and offline indicator behavior
- `hato-fe/src/app/ui/layout/main-layout/sidebar/sidebar.ts` — removes GANADERO manual sync/backups/conflicts navigation entries
- `hato-fe/src/app/ui/layout/main-layout/sidebar/sidebar.spec.ts` — covers hidden GANADERO tooling entries and remaining calendar badge
- `hato-fe/src/app/ui/layout/main-layout/sidebar/sidebar.integration.spec.ts` — covers role-switching menu without GANADERO manual tooling entries
- `hato-fe/src/app/app.routes.ts` — redirects GANADERO manual tooling URLs to dashboard with `canMatch`; keeps admin conflicts route
- `hato-fe/src/app/app.routes.spec.ts` — covers route redirect policy
- `openspec/changes/animal-offline-architecture-v1/tasks.md` — marks Phase 4, Phase 6, and touched-spec update tasks complete for this slice
- `openspec/changes/animal-offline-architecture-v1/apply-progress.md` — cumulative apply progress merged through PR3

## Status
19/24 implementation tasks complete. Ready for next chained slice: Phase 5 animal list/detail/media badges.
