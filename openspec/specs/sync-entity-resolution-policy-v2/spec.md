...
The system MUST publish a versioned policy matrix by `entity` and `opType` that declares allowed resolution actions, UX hints, retry eligibility, and explicit exclusions (`MUST NOT resolve manually` cases), including `HERD_LOT`, `HERD_LOT_ASSIGNMENT`, `HERD_PRODUCTIVITY_LEDGER`, and `HERD_COST_LEDGER`.
