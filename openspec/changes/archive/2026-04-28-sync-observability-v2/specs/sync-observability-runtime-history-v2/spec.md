...
### Requirement: Runtime Snapshot Metrics (FE)

The system MUST expose a runtime sync snapshot per cycle with `trigger`, `startedAt`, `finishedAt`, `totalDurationMs`, and phase durations for `push` and `pull`.
...
- AND values use the same metric names across cycles
...
- AND `in_flight` queue metrics remain visible
